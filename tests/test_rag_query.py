...
def test_rag_query():
    ...
    assert all(isinstance(doc, str) for doc in similar_docs), "Each item in results should be a string."
    assert all(doc in all_original_docs for doc in similar_docs), "Each similar document should be in the list of all original documents."